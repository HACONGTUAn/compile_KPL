...
Object* checkDeclaredLValueIdent(char* name) {
  ...
  case OBJ_FUNCTION:// nếu định danh là tên của 1 hàm
    if (obj != symtab->currentScope->owner) // nếu khác pham vi hiện tại 
      error(ERR_INVALID_IDENT,currentToken->lineNo, currentToken->colNo);// báo lỗi
    break;
  default:
...
void checkIntType(Type* type) {
  ...
  if(type->typeClass != TP_INT){
    error(ERR_TYPE_INCONSISTENCY,currentToken->lineNo,currentToken->lineNo);
  }
