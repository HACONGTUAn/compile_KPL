...
void compileProgram(void) {
  ...
  Object * obj = NULL;
  eat(KW_PROGRAM); // đọc vào kw_program
  eat(TK_IDENT);
  obj = createProgramObject(currentToken->string);// đọc vào name
  eat(SB_SEMICOLON);
...
error(ERR_INVALID_CONSTANT,currentToken->lineNo,currentToken->colNo);
  ...
  case TK_CHAR:
    ...
    break;
  default:
    error(ERR_INVALID_CONSTANT, lookAhead->lineNo, lookAhead->colNo);// lỗi hằng số không hợp lệ
    break;
  }
  return constValue;
}
...
Type* compileType(void) {
  ...
  case TK_IDENT:
    ...
    if (obj == NULL) {
     ...
     // Sao chÃ©p kiá»ƒu tá»« má»™t kiá»ƒu Ä‘Ã£ khai bÃ¡o
    type = duplicateType(obj->typeAttrs->actualType);
    break;
  default:
    error(ERR_INVALID_TYPE, lookAhead->lineNo, lookAhead->colNo);// kiểu không hợp lệ
    break;
  }
  return type;
}
// biên dịch kiểu cơ bản
Type* compileBasicType(void) {
  ...
  case KW_CHAR: 
    ...
    break;
  default:
    error(ERR_INVALID_BASICTYPE, lookAhead->lineNo, lookAhead->colNo);// kiểu không hợp lệ
    break;
  }
  return type;
}
